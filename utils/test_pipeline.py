...
def test_models():
    ...
    print(f"[OK] FedAvg模型创建成功")
    print(f"  - 骨干网络: DenseNet-169")
    print(f"  - 分类器参数: {sum(p.numel() for name, p in model.named_parameters() if 'classifier' in name):,}")
    ...
    print(f"  - Logits输出: {logits.shape}")
    print(f"  - 特征输出: {features.shape}")
